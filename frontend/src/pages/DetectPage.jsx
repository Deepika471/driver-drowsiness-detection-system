...
// REPLACES your existing DetectPage.jsx
// Changes: Added tab switcher (Upload / Live) at the top.
// The Upload tab is 100% identical to your original code.
// The Live tab lazy-loads LiveDetectPage.

import { useState, useRef, useCallback, Suspense, lazy } from 'react';
import { Upload, ImagePlus, X, Zap, AlertCircle, Eye, Video } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import ResultCard from '../components/ResultCard';

// Lazy-load Live page so webcam libs don't load until needed
const LiveDetectPage = lazy(() => import('./LiveDetectPage'));
...
function TabBar({ active, onChange }) {
  ...
  return (
    <div className="flex gap-1 p-1 bg-panel/40 border border-border w-fit mb-10"
      style={{ clipPath: 'polygon(6px 0%, 100% 0%, calc(100% - 6px) 100%, 0% 100%)' }}>
      {tabs.map(({ id, label, icon: Icon }) => (
        <button
          key={id}
          onClick={() => onChange(id)}
          className={`flex items-center gap-2 px-5 py-2.5 font-mono text-xs tracking-widest transition-all duration-200
            ${active === id
              ? 'bg-accent text-void'
...
function UploadModule() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || 'Detection failed. Please try again.');
      setResult(data);
      ...
      <div className="space-y-4">
        <div
          ...
          className={`relative border-2 border-dashed transition-all duration-300 cursor-pointer min-h-64 flex flex-col items-center justify-center overflow-hidden
            ${dragging ? 'border-accent bg-accent/10' : file ? 'border-border cursor-default' : 'border-border hover:border-accent/50 hover:bg-panel/40'}`}
        ...
        >
          ...
          {preview ? (
            <>
              <img src={preview} alt="Preview" className="w-full h-full object-contain max-h-72" />
        ...
        {file && (
          ...
          >
            {loading ? (
              <><span className="w-5 h-5 border-2 border-void/30 border-t-void rounded-full animate-spin" /><span>ANALYZING IMAGE...</span></>
            ) : (
              <><Zap className="w-5 h-5" /><span>RUN ANALYSIS</span></>
            )}
          </button>
        )}
            ...
            <div className="space-y-2">
              {['Loading model weights', 'Preprocessing image', 'Running inference', 'Computing softmax'].map((step, i) => (
                <div key={step} className="flex items-center gap-2">
                  <div className="w-1.5 h-1.5 rounded-full bg-accent animate-pulse" style={{ animationDelay: `${i * 0.4}s` }} />
...
// ── Main DetectPage with tab routing ──────────────────────────
      ...
      <div className="relative max-w-6xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <p className="font-body text-sm text-text-dim mt-2 max-w-lg">
            Upload a driver image or use live webcam for continuous AI-powered drowsiness monitoring.
          </p>
        </div>
          ...
          <Suspense fallback={
            <div className="flex items-center justify-center h-64">
              <div className="flex items-center gap-3">
                ...
                <span className="font-mono text-xs text-accent tracking-widest">LOADING MODULE...</span>
              </div>
            </div>
          }>
            <LiveDetectPage />
