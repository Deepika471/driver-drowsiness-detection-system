...
export function useWebcamDetection(token) {
  ...
  const hasSavedRef = useRef(false); // to prevent multiple saves
  ...
  const triggerDrowsy = useCallback((overrideResult = null, detectedBy = "CNN") => {
    ...
    if (drowsyCountRef.current === ALARM_THRESHOLD) {
      console.log("🔔 Threshold hit! Attempting save. Token exists:", !!token); // ← ADD THIS
      playAlarm();

      if (!hasSavedRef.current) {
        hasSavedRef.current = true; // prevent multiple saves for same event

      // Save this drowsy event to MongoDB (one record per event, not per frame)
      const eventData = overrideResult || {};
      console.log("📤 Sending to save-drowsy-event:", eventData); // ← ADD THIS
